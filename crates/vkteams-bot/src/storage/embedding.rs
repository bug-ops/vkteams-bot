...
pub trait EmbeddingClient: Send + Sync {
    ...
    async fn generate_embeddings_parallel(&self, texts: &[String]) -> Result<Vec<Vec<f32>>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn generate_embeddings_chunked(&self, texts: &[String], chunk_size: usize) -> Result<Vec<Vec<f32>>, Box<dyn std::error::Error + Send + Sync>> {
...
impl EmbeddingClient for OpenAIEmbeddingClient {
    ...
    async fn generate_embeddings(&self, texts: &[String]) -> Result<Vec<Vec<f32>>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Some(data) = response_json["data"].as_array() {
            ...
            let embeddings: Result<Vec<Vec<f32>>, Box<dyn std::error::Error + Send + Sync>> = data
                .par_iter()
                .map(|item| -> Result<Vec<f32>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn generate_embeddings_chunked(&self, texts: &[String], chunk_size: usize) -> Result<Vec<Vec<f32>>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let chunk_futures = chunks.into_iter().map(|chunk| {
            ...
            async move {
                client.generate_embeddings(&chunk_texts).await
            }
        });
...
impl EmbeddingClient for OllamaEmbeddingClient {
    ...
    async fn generate_embeddings_chunked(&self, texts: &[String], chunk_size: usize) -> Result<Vec<Vec<f32>>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let chunk_futures = chunks.into_iter().map(|chunk| {
            ...
            async move {
                client.generate_embeddings_parallel(&chunk_texts).await
            }
        });
...
pub mod processing {
    ...
    pub async fn process_large_batch(
        ...
        match (config.provider_specific_settings.clone(), texts.len()) {
            (ProviderSettings::OpenAI { .. }, n) if n > config.chunk_size => {
                ...
                client.generate_embeddings_chunked(texts, config.chunk_size).await
            },
    ...
    pub fn estimate_optimal_chunk_size(
        ...
        let avg_length: f64 = texts
            .par_iter()
            .map(|text| text.len())
            .sum::<usize>() as f64 / texts.len() as f64;
...
#[cfg(all(test, feature = "ai-embeddings"))]
mod tests {
    use super::*;
