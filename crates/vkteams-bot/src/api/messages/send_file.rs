...
mod tests {
    ...
    fn test_serialize_deserialize_request_minimal() {
        let req = RequestMessagesSendFile::new((
            ChatId("c1".to_string()),
            MultipartName::File("file_id".to_string()),
        ));
        ...
        match req2.multipart {
            MultipartName::File(ref s) => assert_eq!(s, "file_id"),
    ...
    fn test_serialize_deserialize_response() {
        ...
        assert_eq!(val["fileId"], "file_id");
        let resp2: ResponseMessagesSendFile = serde_json::from_value(val).unwrap();
        assert_eq!(resp2.msg_id.as_ref().unwrap().0, "m1");
        assert_eq!(resp2.file_id.as_ref().unwrap(), "file_id");
    }

    #[test]
    fn test_request_missing_required_field() {
        let val = serde_json::json!({"text": "hello"});
        let req = serde_json::from_value::<RequestMessagesSendFile>(val);
        assert!(req.is_err());
    }
}
