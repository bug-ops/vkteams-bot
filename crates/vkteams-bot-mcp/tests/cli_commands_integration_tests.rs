...
fn test_command_argument_patterns() {
    ...
    let messaging_commands = vec![
        ...
        (
            "send-text",
            vec!["--message", "Reply", "--reply-msg-id", "456"],
        ),
        ("send-file", vec!["--file-path", "/path/file.txt"]),
        (
            "send-file",
            vec!["--file-path", "/path/file.txt", "--caption", "Test"],
        ),
        ("send-voice", vec!["--file-path", "/path/voice.ogg"]),
        (
            "send-voice",
            vec!["--file-path", "/path/voice.ogg", "--chat-id", "789"],
        ),
    ];
...
fn test_message_management_command_patterns() {
    let message_commands = vec![
        (
            "edit-message",
            vec!["--message-id", "123", "--new-text", "Updated"],
        ),
        (
            "edit-message",
            vec![
        ...
        (
            "delete-message",
            vec!["--message-id", "123", "--chat-id", "456"],
        ),
    ...
    for (command, args) in message_commands {
        ...
        assert!(
            full_args
                .iter()
                .any(|arg| arg.parse::<i32>().is_ok() || arg.contains("msg"))
        );
        ...
        if full_args.contains(&"--chat-id") {
            ...
            assert!(
                chat_idx + 1 < full_args.len(),
                "chat-id should have a value"
            );
        }
    }
}
...
fn test_chat_management_command_patterns() {
    let chat_commands = vec![
        ...
        (
            "get-chat-members",
            vec!["--chat-id", "123", "--cursor", "cursor123"],
        ),
        ("get-chat-admins", vec![]),
        ...
        (
            "set-chat-title",
            vec!["--title", "New Title", "--chat-id", "123"],
        ),
        ("set-chat-about", vec!["--about", "New description"]),
        (
            "set-chat-about",
            vec!["--about", "New description", "--chat-id", "123"],
        ),
    ];
...
fn test_action_and_upload_command_patterns() {
    let action_commands = vec![
        ...
        (
            "send-action",
            vec!["--action", "typing", "--chat-id", "123"],
        ),
        ("send-action", vec!["--action", "looking"]),
        (
            "upload-file-base64",
            vec!["--name", "test.txt", "--content-base64", "dGVzdA=="],
        ),
        (
            "upload-file-base64",
            vec![
        ...
        (
            "upload-text-file",
            vec!["--name", "notes.txt", "--content", "Text content"],
        ),
        (
            "upload-json-file",
            vec!["--name", "data", "--json-data", r#"{"key": "value"}"#],
        ),
    ];
...
fn test_database_and_search_command_patterns() {
    let db_commands = vec![
        ...
        (
            "get-database-stats",
            vec!["--chat-id", "123", "--since", "2024-01-01"],
        ),
        ("search-semantic", vec!["--query", "test query"]),
        (
            "search-semantic",
            vec!["--query", "test query", "--chat-id", "123"],
        ),
        (
            "search-semantic",
            vec!["--query", "test query", "--limit", "10"],
        ),
        ...
        (
            "get-context",
            vec!["--chat-id", "123", "--limit", "50", "--timeframe", "1d"],
        ),
    ];
...
fn test_utility_and_events_command_patterns() {
    let utility_commands = vec![
        ...
        (
            "events-get",
            vec!["--last-event-id", "123", "--timeout", "30"],
        ),
    ...
    for (command, args) in utility_commands {
        ...
        if full_args.contains(&"--timeout") {
            ...
            assert!(
                timeout_value.parse::<i32>().is_ok(),
                "Timeout should be numeric"
            );
        }

        if full_args.contains(&"--last-event-id") {
            let event_idx = full_args
                .iter()
                .position(|&x| x == "--last-event-id")
                .unwrap();
    ...
    for response in success_responses {
        ...
        if let Some(data) = response["data"].as_object() {
            for (key, value) in data {
                ...
                assert!(
                    !value.is_null() || key.contains("cursor"),
                    "Most fields should not be null"
                );
            }
        }
    }
...
fn test_parameter_validation_patterns() {
    ...
    for (command, required_params) in required_parameters {
        ...
        assert!(
            ...
            "Command {command} should have required parameters"
        );
        ...
        for param in required_params {
            ...
            assert!(
                !param.contains(" "),
                "Parameter name should not contain spaces"
            );
            assert!(
                param.contains("-") || param.chars().all(|c| c.is_ascii_lowercase()),
    ...
    for param in optional_parameters {
        assert!(
            !param.is_empty(),
            "Optional parameter name should not be empty"
        );
        assert!(
            !param.contains(" "),
            "Optional parameter name should not contain spaces"
        );
    }
}
...
fn test_file_path_and_id_patterns() {
    ...
    for file_id in file_ids {
        ...
        assert!(
            file_id.len() >= 3,
            "File ID should be at least 3 characters"
        );
    }
}
...
fn test_numeric_parameter_validation() {
    ...
    for (param_name, values) in numeric_params {
        for value in values {
            ...
            assert!(
                parsed.is_ok(),
                "Value {value} should be a valid number for {param_name}"
            );

            let num = parsed.unwrap();
            assert!(
                num >= 0,
                "Numeric value should be non-negative for {param_name}"
            );
            ...
            if param_name == "timeout" {
                assert!(
                    (1..=600).contains(&num),
                    "Timeout should be between 1 and 600 seconds"
                );
            }
        }
    }
...
fn test_boolean_flag_patterns() {
    ...
    for (flag_name, values) in boolean_flags {
        for value in values {
            ...
            assert!(
                is_boolean,
                "Value {value} should be a valid boolean for {flag_name}"
            );

            let parsed = value.parse::<bool>();
            assert!(
                parsed.is_ok(),
                "Value {value} should parse as boolean for {flag_name}"
            );
        }
    }
}
