...
use vkteams_bot_cli::commands::daemon::{AutoSaveEventProcessor, DaemonCommands};
...
fn test_processor_stats_serialization() {
    ...
    let processor = rt.block_on(async { AutoSaveEventProcessor::new(&config).await.unwrap() });
...
fn test_processor_stats_structure() {
    ...
    assert!(
        ...
        "events per second should be non-negative"
    );
